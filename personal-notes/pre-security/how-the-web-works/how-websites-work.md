...
Sensitive information can be potentially leveraged to further an attacker's access within different parts of a web application. For example, an engineer may have left a HTML comment with temporary login credentials, and if you viewed the page's source code and found this, you could use these credentials to log in on the application (or worse, the credentials are used to access other backend components of the site).
